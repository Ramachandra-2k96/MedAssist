  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (role === 'doctor') {
          window.location.href = `/doctor-dashboard/${btoa(email).replace(/[^a-zA-Z0-9]/g, '').substring(0, 32)}`
        } else {
          window.location.href = `/dashboard/${btoa(email).replace(/[^a-zA-Z0-9]/g, '').substring(0, 32)}`
        }
      } else {
