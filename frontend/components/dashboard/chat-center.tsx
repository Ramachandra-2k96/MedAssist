...
export function ChatCenter({ mode, patientId, doctorId }: ChatCenterProps) {
  ...
  const fetchMessages = async () => {
    ...
    } catch (e: any) { console.error(e); toast.error('Failed to load chat', { description: e?.detail ? JSON.stringify(e.detail) : String(e) }) } finally { setLoading(false) }
  }

  const sendMessage = async () => {
    if (!input.trim()) return
    if (mode === 'patient' && !doctorId) { toast.error('Select a doctor', { description: 'Choose a doctor to send a message.' }); return }
    ...
    try {
      ...
      fetchMessages()
    } catch (e: any) { console.error(e); toast.error('Send failed', { description: e?.detail ? JSON.stringify(e.detail) : String(e) }) }
  }
